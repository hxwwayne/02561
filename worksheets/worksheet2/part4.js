...
function drawCircle(cx, cy, radius) {
    ...
    for (var i = 0; i < numSegments; i++) {
        ...
        // Each segment is a triangle (cx, cy, x1, y1, x2, y2)
        vertices.push(cx, cy);
        vertices.push(cx + radius * Math.cos(angle), cy + radius * Math.sin(angle));
        vertices.push(cx + radius * Math.cos(nextAngle), cy + radius * Math.sin(nextAngle));
        for (let j = 0; j < 3; j++) { // Add color for each vertex of the triangle
            colors.push([currentPointColor.r / 255, currentPointColor.g / 255, currentPointColor.b / 255]);
        }
    }
}
