...
fn compute_shadow_factor(lightSpacePos : vec4<f32>) -> f32 {
    ...
    // 超出 [0,1] 视锥外：不在 shadow map 范围内，认为“无阴影”
    ...
    // shadow map 中的深度 < 当前点深度 - epsilon → 在阴影里
...
fn fs_main_ground(input: VertexOutputGround) -> @location(0) vec4<f32> {
    ...
    // 简单一点：ambient + shadowed direct 光照
    let Le  = uniforms.params1.x;
    let La  = uniforms.params1.y;
    let kd  = uniforms.params1.z;
    ...
    let color = texColor.rgb * lighting * visibility;
...
fn vs_main_depth(input: VSInDepth) -> VSOutDepth {
    ...
    out.position       = lightClip;      // 写到光源视角的裁剪空间
    out.lightSpacePos  = lightClip;      // 顺便传给 fragment 用来算 z
...
fn fs_main_depth(input : FSInDepth) -> @location(0) vec4<f32> {
    let p = input.lightSpacePos / input.lightSpacePos.w;   // NDC [-1,1]
    let depth = 0.5 * p.z + 0.5;                           // 映射到 [0,1]
